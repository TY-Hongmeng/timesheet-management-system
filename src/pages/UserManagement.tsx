...
import { User, Plus, Edit, Trash2, Search, Save, X, Shield, Phone, CreditCard, Building, UserCheck, AlertTriangle, RefreshCw, Users, UserX } from 'lucide-react'
...
export default function UserManagement() {
  ...
  // 注册默认状态控制 - 简化初始化
  const [defaultUserStatus, setDefaultUserStatus] = useState<boolean>(false)
  ...
  useEffect(() => {
    const runDiagnostics = async () => {
      console.log('🔍 [Diagnostics] 开始全面存储诊断检查')
      const results: string[] = []
      let newDiagnosticInfo = { ...diagnosticInfo }

      // 1. 检测浏览器信息
      const browserInfo = `${navigator.userAgent} | ${navigator.platform} | ${navigator.language}`
      newDiagnosticInfo.browserInfo = browserInfo
      results.push(`✅ 浏览器信息: ${browserInfo.substring(0, 100)}...`)

      // 2. 检测 localStorage 支持
      try {
      ...
      try {
        ...
        if (testValue === 'test') {
          newDiagnosticInfo.sessionStorageSupported = true
          results.push('✅ sessionStorage 基础功能正常')
        } else {
          throw new Error('sessionStorage 读写不一致')
        }
      } catch (error) {
        newDiagnosticInfo.sessionStorageSupported = false
        results.push(`❌ sessionStorage 不可用: ${error}`)
      }
      ...
      try {
        ...
        localStorage.setItem('__privacy_test', testData)
        localStorage.removeItem('__privacy_test')
        newDiagnosticInfo.isPrivateMode = false
        results.push('✅ 非隐私模式，存储配额正常')
      } catch (error) {
        if (error instanceof DOMException && error.code === 22) {
          newDiagnosticInfo.isPrivateMode = true
          newDiagnosticInfo.storageQuotaExceeded = true
          results.push('⚠️ 可能处于隐私模式或存储配额已满')
        } else {
          results.push(`⚠️ 存储配额检测异常: ${error}`)
        }
      }

      // 5. 检测当前 defaultUserStatus 状态
      const currentValue = localStorage.getItem('defaultUserStatus')
      results.push(`🔍 当前 defaultUserStatus: ${currentValue || 'null'}`)

      // 6. 尝试多种方式设置测试值
      const testMethods = [
        { name: 'JSON.stringify(true)', value: JSON.stringify(true) },
        { name: 'String(true)', value: String(true) },
      ...
      for (const method of testMethods) {
        try {
          localStorage.setItem('__test_method', method.value)
          const retrieved = localStorage.getItem('__test_method')
          localStorage.removeItem('__test_method')
          
          if (retrieved === method.value) {
            results.push(`✅ 测试方法 ${method.name}: 成功`)
          } else {
            results.push(`❌ 测试方法 ${method.name}: 失败 (期望: ${method.value}, 实际: ${retrieved})`)
          }
        } catch (error) {
          results.push(`❌ 测试方法 ${method.name}: 异常 ${error}`)
        }
      }

      newDiagnosticInfo.storageTestResults = results
      setDiagnosticInfo(newDiagnosticInfo)
      
      console.log('🔍 [Diagnostics] 诊断完成:', results)
    }
  ...
  // 按分公司分组用户
  const groupedUsers = filteredUsers.reduce((groups, userData) => {
    const companyName = userData.company?.name || '未分配公司'
    if (!groups[companyName]) {
      groups[companyName] = []
    }
    groups[companyName].push(userData)
    return groups
  }, {} as Record<string, UserData[]>)

  // 对分组进行排序（公司名称排序）
  const sortedCompanyNames = Object.keys(groupedUsers).sort()
  ...
  return (
    <div className="min-h-screen bg-black text-green-300 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Users Table - Grouped by Company */}
            ...
            {sortedCompanyNames.map((companyName) => {
              ...
              return (
                <CollapsibleSection
                  ...
                  className="bg-gray-900 border border-green-400 rounded-lg overflow-hidden"
                >
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="bg-green-900/30">
                        <tr>
                          <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">序号</th>
                          <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">姓名</th>
                          <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">手机号</th>
                          <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">身份证号</th>
                          <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">角色</th>
                          <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">生产线</th>
                          <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">状态</th>
                          <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">操作</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-green-800">
                        {companyUsers.map((userData, index) => (
              ...
              <table className="w-full">
                <thead className="bg-green-900/30">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">手机号</th>
                    <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">身份证号</th>
                    <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">公司</th>
                    <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">角色</th>
                    <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">生产线</th>
                    <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">状态</th>
                    <th className="px-4 py-3 text-left text-sm font-mono font-medium text-green-400 uppercase tracking-wider">操作</th>
